...
function _newGame() {
    gameMatrix = [[0,0,0],[0,0,0],[0,0,0]]; //filled with zeros as needed
    gameOver = false;
}
...
function didSome1Win(three) {
    ...
    for (var x = 0; x <= 7; x++) {
        if (arraySum(gameMatrix, x) === three) {
            if (x >= 0 && x <= 2) {
                for (var i = 0; i < 3; i++) {
                    divMatrix[x][i].style.backgroundColor = "#a663ff";
                }
            } else if (x >= 3 && x <= 5) {
                for (var i = 0; i < 3; i++) {
                    divMatrix[i][x - 3].style.backgroundColor = "#a663ff";
                }
            } else if (x === 6) {
                for (var i = 0; i < 3; i++) {
                    divMatrix[i][i].style.backgroundColor = "#a663ff";
                }
            } else if (x === 7) {
                for (var i = 0; i < 3; i++) {
                    divMatrix[2 - i][i].style.backgroundColor = "#a663ff";
                }
            }
            removeAllClicks();
            gameOver = true;
